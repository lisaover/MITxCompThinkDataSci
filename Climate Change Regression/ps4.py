...
def evaluate_models_on_training(x, y, models):
    ...
    for m in models:
        ...
        pylab.figure()
        pylab.plot(x, y, 'b:', linewidth=3, label = 'Actual values')
        estY = pylab.polyval(m, x)
        pylab.plot(x, estY, '-r', linewidth=1, label='Predicted values')
        pylab.legend(loc = "best")
        if len(m) == 2:
            R2 = r_squared(y, estY)
            print(R2)
            pylab.title('Temperatures Modeled as Degree ' + str(len(m)-1) + '\n' + 'with R-sqaured = ' + str(R2))
        else:
            pylab.title('Temperatures Modeled as Degree ' + str(len(m)-1))
        pylab.xlabel('Year')
...
#x = [1961, 1962, 1963]
#y = [4.4,5.5,6.6]
#x = [32.0, 42.0, 31.3, 22.0, 33.0]
#y = [32.3, 42.1, 31.2, 22.1, 34.0]
